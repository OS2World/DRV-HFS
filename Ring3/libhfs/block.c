...
/* 
 * Modified for HFS/2 by Marcus Better, April 1997.
 */
...
{
  return read_blocks(vol->vstart + num, 1, HFS_BLOCKSZ, vol->sector_size,
		     vol->hVPB, bp);
}
#else
int b_readlb(hfsvol *vol, unsigned long num, block *bp)
...
{
  return write_blocks(vol->vstart + num, 1, HFS_BLOCKSZ, vol->sector_size,
		      vol->hVPB, bp);
}
#else
int b_writelb(hfsvol *vol, unsigned long num, block *bp)
