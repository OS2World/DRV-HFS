...
/* Translate Macintosh filename to OS/2 filename. Nonzero return value means 
   filename is invalid. */
...
static int os2_to_mac_name_cp850(const unsigned char *src, unsigned char *dest)
{
  ...
  for(i=0; i<strlen(src) && p-component<=MAX_MAC_FN_LEN; i++) {
    unsigned char c=src[i];
